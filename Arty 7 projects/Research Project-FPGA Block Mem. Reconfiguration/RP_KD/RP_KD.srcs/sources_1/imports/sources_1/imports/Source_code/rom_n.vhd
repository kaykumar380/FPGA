...
architecture rtl of ROM52 is
	signal A_r,A_r_s: std_logic_vector(12 downto 0):= "0000000000000"; 
	signal rom_in: std_logic_vector (7 downto 0);
	type type_opcode is array (0 to (2**rom_size-1)) of std_logic_vector(7 downto 0);
...
begin
	...
	begin
		if Clk'event and Clk = '1' then
			--A_r <= "00" & A(10 downto 0); --------------------------------remove "00" if 64kByte ROM is used
			A_r((rom_size-1) downto 0) <= A((rom_size-1) downto 0); -----kkk
		end if;
	end process;
	-------------------------
read_op: process(A_r)
  begin
	...
	begin
	     if progk='1' and read_now'event and read_now ='1' then
                 A_r_s((rom_size-1) downto 0) <= A((rom_size-1) downto 0);  ----------------------------------------kkkkkkkk
                 rom_in <= Din;
         end if;
	end process;
